...
async function sendDesktopNotification(options: NotificationOptions): Promise<void> {
	...
	await sendDesktopNotificationByPlatform({
		...
		sound,
		senderBundleId: undefined,
		tmuxPaneId: options.tmuxPaneId,
		tmuxWindowId: options.tmuxWindowId,
...
async function handleSessionIdle(
	...
	try {
		...
		if (session.data?.title) {
			sessionTitle = session.data.title.slice(0, 60)
		}
	} catch {
		// Use default title
	}
	...
	try {
		...
		const messages = Array.isArray(msgs?.data) ? msgs.data : []
		let lastUser: string | null = null
		let lastAssistant: string | null = null
		for (const msg of messages) {
			const role = (msg.info as { role?: string }).role
	...
	let message = ""
	if (question && answer) {
		message = `Q: ${question.slice(0, 200)}\nA: ${answer.slice(0, 200)}`
	} else if (question) {
